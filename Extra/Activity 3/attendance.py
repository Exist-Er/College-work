...
def mark_attendance(image_path):
    ...
    # Get all student names from training directory for the full class list
    all_students = sorted([d for d in os.listdir(TRAIN_DIR) if os.path.isdir(os.path.join(TRAIN_DIR, d))])
    ...
    if not face_encodings:
        ...
        # Predict names for each face detected
    ...
    for student in all_students:
        ...
        attendance_data.append({"Student Name": student, "Status": status, "Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
